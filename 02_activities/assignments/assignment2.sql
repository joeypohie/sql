...
CREATE TEMPORARY TABLE sales_by_date AS 
	SELECT market_date, (quantity*cost_to_customer_per_qty) AS daily_sales
	FROM customer_purchases
...
CREATE TEMPORARY TABLE ranked_sales AS
	SELECT
		market_date, 
		daily_sales,
		RANK() OVER (ORDER BY daily_sales DESC) as highest_rank,
		RANK() OVER (ORDER BY daily_sales ASC) as lowest_rank
	FROM sales_by_date;
...
SELECT *
FROM product_units
WHERE product_name IS 'Apricot Pie';
...
WHERE product_id = 999 AND product_name = 'Apricot Pie';
...
CREATE TEMP TABLE temp_latest_quantity AS
SELECT
	product_id,
...
SET current_quantity = tlq.latest_quantity
FROM temp_latest_quantity AS tlq
WHERE product_units.product_id = tlq.product_id;